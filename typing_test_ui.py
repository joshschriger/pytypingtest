'''
I think this will be my main file???
...
# TODO create Typing test class with test type, word dict, etc 
# NEEDS WORK
class Test:
...
def generate_words(word_total):
    """
    Generate the word bank from the word dict
    """
    if Test.test_type == 'Timed':
...
def start_action():
    ...
    generate_words(int(word_total))


def typing_test(test_type, start_time):
    """
...
def key_pressed(event):
    ...
    if to_type['text'] == 'To test your typing speed, press enter and start typing!':
        messagebox.showwarning(
            title='Error', 
            message='You must generate a new word bank before testing\nTo do so, press enter or click the Start Game button')
        return 0
    if not pressed_once:
        pressed_once = True
        start_timer(get_test_type())


def start_timer(test_type):
    """
    Starts game's timer
    """
    time_start = time.time()
    typing_test(test_type, time_start)
...
slider = Scale(orient='vertical', from_=60, to=10, # TODO variable=word_total,
